...
The container belongs to the Kekkai Runtime process. Kekkai Runtime starts `runc` or `runsc` in foreground mode, waits for accepted execution tasks during graceful shutdown, then kills and deletes the container and removes its session veth. The bridge and iptables rules are retained as shared, idempotently managed host resources. A stale container left by an unclean termination is removed at the next startup; Linux parent-death protection also makes the foreground runtime follow Kekkai Runtime when the process is forcefully terminated.
...
kekkai-rt init alpine

# Pin a specific Alpine stable release
kekkai-rt init alpine --version 3.24.1
```
